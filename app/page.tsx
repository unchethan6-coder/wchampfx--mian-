...
export default function Page() {
  ...
  return (
    <>
      ...
      <footer><div className="container footer-top"><div><a className="brand" href="#top">WCHAMPFX</a><p className="micro footer-copy">A structured path for serious prop traders.</p></div><div className="footer-links"><a href="#proof">Results</a><a href="#dan">About Dan</a><a href="#learn">Free Training</a><a href="#top">Privacy</a></div></div><div className="container disclaimer">*All results and payout references are illustrative and not a guarantee of future performance. Trading and prop firm evaluations involve significant risk. Educational content only.</div></footer>
      <div className="mobile-sticky"><button className="btn" onClick={scrollToForm}>GET FREE ROADMAP</button></div>
      {showExit && <div className="popup-backdrop show" role="presentation"><div className="popup" role="dialog" aria-modal="true" aria-labelledby="popup-title"><button className="popup-close" aria-label="Close" onClick={() => setShowExit(false)}>×</button><span className="eyebrow">Before you go</span><h3 id="popup-title">Want the roadmap before you leave?</h3><p>Get the exact framework used to connect strategy, risk, funding and scale. It&apos;s free and takes less than 30 seconds.</p><button className="btn full" onClick={() => { setShowExit(false); scrollToForm() }}>Send me the free roadmap</button></div></div>}
    </>
  )
}
