...
export default function Page() {
  ...
  function openModal(event?: { currentTarget: HTMLElement }) {
  ...
  return (
    <>
      ...
      <main id="top">
        ...
        <section className="section" id="dan"><div className="container authority"><div className="dan-visual"><div className="portrait-mark">DC</div><div className="dan-copy"><span className="micro">THE PERSON BEHIND THE PROCESS</span><strong>Dan Cheung</strong><span className="muted">Trader, coach and founder of WChampFX.</span></div></div><div><span className="eyebrow">Built from experience</span><h2>Structure beats <span className="accent">noise.</span></h2><p className="lead">Dan built WChampFX for traders who want a repeatable path through the prop funding journey — not another collection of disconnected strategies.</p><div className="stat-list"><div className="stat"><b>$500K+</b><span>verified payouts*</span></div><div className="stat"><b>347+</b><span>students inside WChampFX</span></div><div className="stat"><b>1 process</b><span>from strategy to scale</span></div><div className="stat"><b>100%</b><span>free roadmap access</span></div></div><button className="btn" onClick={openModal}>Get the framework</button></div></div></section>
        ...
        <section className="section-sm"><div className="container final"><div><span className="eyebrow">Your roadmap is free</span><h2>Stop guessing what to <span className="accent">fix next.</span></h2><p className="lead">Get the WChampFX $1M Prop Funding Roadmap and see the full process in one place.</p></div><button className="btn" onClick={openModal}>GET THE FREE ROADMAP →</button></div></section>
      <aside className="floating-vsl" aria-label="Featured WChampFX training video"><div className="floating-vsl-head"><span>WChampFX VSL</span><button type="button" aria-label="Close video" onClick={(event) => event.currentTarget.closest('.floating-vsl')?.remove()}>×</button></div><div className="floating-vsl-frame"><iframe src="https://www.youtube.com/embed/wIssK9Kqums?autoplay=1&mute=1&loop=1&playlist=wIssK9Kqums&controls=1&rel=0&playsinline=1" title="WChampFX featured training video" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div></aside></main>
      ...
      <div className="mobile-sticky"><button className="btn" onClick={openModal}>GET FREE ROADMAP</button></div>
      {showExit && <div className="popup-backdrop show" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal() }}><div className="popup popup-lead" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="popup-title" aria-describedby="popup-description" tabIndex={-1}><button className="popup-close" aria-label="Close roadmap popup" onClick={closeModal}>×</button><div className="popup-media"><div><span className="micro">FEATURED TRAINING</span><strong>Dan Cheung</strong></div><div className="play" aria-hidden="true">▶</div></div><div className="popup-content" id="popup-description">{!submitted ? <><span className="eyebrow">Before you go</span><h3 id="popup-title">Get the roadmap free</h3><p>Enter your details, then answer 6 quick questions to build your personal roadmap.</p><form onSubmit={handleSubmit}><div className="popup-form-grid"><Field label="First Name" name="firstName" placeholder="First name"/><Field label="Email" name="email" type="email" placeholder="you@email.com"/><Field label="WhatsApp" name="whatsapp" type="tel" placeholder="+971..."/><label className="field">Country<select name="country" required><option value="">Select country</option><option>United Kingdom</option><option>United States</option><option>Germany</option><option>United Arab Emirates</option><option>Australia</option><option>Canada</option><option>Other</option></select></label><button className="btn full popup-submit" type="submit">START THE 6 QUESTIONS →</button></div></form><div className="form-meta micro">Free access. Takes less than 30 seconds.</div><div className="risk-note">Educational content only. Trading and prop firm evaluations involve risk.</div></> : showRoadmap ? <div className="popup-result"><span className="eyebrow">Your roadmap is ready</span><h3>Start with your personal next step.</h3><p>Based on your answers, we&apos;ll tailor the WChampFX funding roadmap and send the next steps to your inbox.</p><div className="roadmap-result"><span className="micro">YOUR JOURNEY</span><strong>{quizAnswers[3] || 'Build a repeatable process'}</strong><span className="muted">Focus on the stage that will create the most progress next.</span></div><a className="btn full result-cta" href="https://learn.fundedproptraders.com/store?_gl=1*55hvzj*_ga*MjU0NTkwNDY1LjE3ODM2MTU1NjY.*_ga_RZG9Q73BBL*czE3ODg3MTY1MTgkbzQkZzEkdDE3ODg3MTkxMjIkajYwJGwwJGgw">CONTINUE TO THE TRAINING →</a></div> : <div className="popup-quiz"><div className="quiz-heading"><span className="eyebrow">Build Your Personal Funding Roadmap</span><h3>{quizQuestions[quizStep][0]}</h3><p>Question {quizStep + 1} of {quizQuestions.length} · Approximately 60 seconds</p></div><div className="quiz-progress"><span>Progress</span><span>{Math.round(((quizStep + 1) / quizQuestions.length) * 100)}%</span></div><div className="quiz-bar"><span style={{ width: `${((quizStep + 1) / quizQuestions.length) * 100}%` }} /></div><div className="quiz-options">{(quizQuestions[quizStep][1] as string[]).map((answer) => <button className="quiz-option" key={answer} onClick={() => answerQuiz(answer)}>{answer}<span aria-hidden="true">→</span></button>)}</div></div>}</div></div></div>}
    </>
  )
}
