...
export default function Page() {
  ...
  return (
    <>
      ...
      {showExit && <div className="popup-backdrop show" role="presentation"><div className="popup popup-lead" role="dialog" aria-modal="true" aria-labelledby="popup-title"><button className="popup-close" aria-label="Close" onClick={() => setShowExit(false)}>×</button><div className="popup-media"><div><span className="micro">FEATURED TRAINING</span><strong>Dan Cheung</strong></div><div className="play" aria-hidden="true">▶</div></div><div className="popup-content"><span className="eyebrow">Before you go</span><h3 id="popup-title">Get the roadmap free</h3><p>See the exact framework used to connect strategy, risk, funded accounts and scale.</p><form onSubmit={(event) => { handleSubmit(event); setShowExit(false) }}><div className="popup-form-grid"><Field label="First Name" name="firstName" placeholder="First name"/><Field label="Email" name="email" type="email" placeholder="you@email.com"/><Field label="WhatsApp" name="whatsapp" type="tel" placeholder="+971..."/><label className="field">Country<select name="country" required><option value="">Select country</option><option>United Kingdom</option><option>United States</option><option>Germany</option><option>United Arab Emirates</option><option>Australia</option><option>Canada</option><option>Other</option></select></label><button className="btn full popup-submit" type="submit">GET THE FREE ROADMAP →</button></div></form><div className="form-meta micro">Free access. Takes less than 30 seconds.</div><div className="risk-note">Educational content only. Trading and prop firm evaluations involve risk.</div></div></div></div>}
    </>
  )
}
